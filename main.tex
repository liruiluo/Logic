...
\tikz[remember picture,overlay] {
  % 背景渐变
  \fill[LogicLightBlue] (current page.north west) rectangle (current page.south east);

  % 顶部装饰条
  \fill[LogicBlue] (current page.north west) rectangle ([yshift=-1cm]current page.north east);

  % 底部装饰条
  \fill[LogicDarkBlue] ([yshift=1cm]current page.south west) rectangle (current page.south east);

  % 几何装饰元素
  \foreach \i in {1,2,3,4,5} {
    \fill[LogicBlue!20] ([xshift=\i*3cm,yshift=-\i*0.5cm]current page.north east) circle (0.3cm);
  }
}
...
% 主标题
{\fontsize{48}{58}\selectfont\cntitle\color{LogicDarkBlue} 逻辑学导论 \par}
\vspace{1cm}

% 副标题
{\Large\color{LogicBlue}\textit{系统梳理逻辑学基本概念与方法} \par}
\vspace{0.5cm}

% 装饰线
\tikz{\draw[LogicBlue,line width=2pt] (0,0) -- (8cm,0);}
...
\centering
\large\color{LogicGray}\textit{
"逻辑将使人们明辨是非，认清谬误，\\[0.5em]
培养批判性思维，提高分析能力。"
}
\end{tcolorbox}
...
\node[
  ...
  minimum width=6cm,
  minimum height=1cm
] {专业教材 \textbullet\ 系统全面 \textbullet\ 实用性强};
\end{tikzpicture}

\vfill

% 日期
{\large\color{LogicGray} \today \par}
\vspace{1cm}
\end{titlepage}
